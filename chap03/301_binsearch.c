...
#define MAX_ELEMENT 1<<21
#define NR_ITER 10000000

/* Outputs approximation of processor time required for our two binary search
 * functions. We search for the element -1, to time the functions' worst case
 * performance (i.e. element not found in test data)
 *
 * Kudos to Paul Griffiths who wrote the original version of this solution
 * which can be found on the following link:
 *
 *   http://users.powernet.co.uk/eton/kandr2/krx301.html
 */
...
int main(void) {
    ...
    for ( i = 0, time_taken = clock(); i < NR_ITER; ++i ) {
        index = binsearch(i, testdata, MAX_ELEMENT);
    }
    ...
    printf("binsearch() took %lu clocks (%lf seconds)\n",
           (unsigned long) time_taken,
           time_taken * 1.0 / CLOCKS_PER_SEC);
    ...
    printf("binsearch2() took %lu clocks (%lf seconds)\n",
           (unsigned long) time_taken,
           time_taken * 1.0 / CLOCKS_PER_SEC);
